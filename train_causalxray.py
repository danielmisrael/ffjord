...
parser.add_argument("--data", choices=['xray', "mnist", "svhn", "cifar10", 'lsun_church'], type=str, default="mnist")
...
parser.add_argument("--save", type=str, default="ffjord/experiments/xray")
...
class XrayCausalDataset(Dataset):
    def __init__(self, interventional=False, split='train', download=False):
    ...
    def __getitem__(self, n):
        # /home/ray/Documents/variational-backdoor/vbdata/causal_xray/train
        patient = Image.open(f'variational-backdoor/vbdata/causal_xray/{self.split}/xray_{n}_z.png')
        # patient, label = self.base_dataset.__getitem__(n)
        Z = add_noise(tforms.ToTensor()(patient))
        label = 0
        return Z, label
...
def get_dataset(args):
    ...
    if args.data == "mnist":
        ...
        im_dim = 1
        im_size = 28
        train_set = XrayCausalDataset(split='train')
        test_set = XrayCausalDataset(split='test')
